			...
			if (ev)
			{
                ...
                User.findOne(e.ownedby[0]).exec(function(err,user){
                    ...
                    Media.find({event_id:e.id}).exec(function(err,media){
                        ...
                        e.iconbackground = (e.iconbackground && e.iconbackground!='') ? sails.config.master_url+'/event/backgroundurl/'+e.id : '';
			...
			Event.find(
				...
				_.each(data,function(d)
				{
					if (d.groupevent)
					{
						...
						_.find(output,{group:mm.profile.displayName}).events.push({status:d.status,iconbackground:(d.iconbackground && d.iconbackground!='') ? sails.config.master_url+'/event/backgroundurl/'+d.id : '',icon:(d.icon && d.icon!='') ? sails.config.master_url+'/event/iconurl/'+d.id : '',name:d.name,id:d.id,starts:d.starts,ends:d.ends,ends_time:d.ends_time,starts_time:d.starts_time, description:d.eventtype.description,offlinecode:d.offlinecode});
					}
					else
					{
            ...
            Event.findByListBuildVariant(req,events,function(err,all)
            {
                User.find({}).exec(function(err,users){
                    events = _.map(all,function(d){
                        var mm = _.find(users,{id:d.ownedby[0]});
                        return {
                            iconbackground:(d.iconbackground && d.iconbackground!='') ? sails.config.master_url+'/event/backgroundurl/'+d.id : '',
		...
		Event.findOne({offlinecode:req.param('shortlink')}).exec(function(err,event){
			if (event && event.public)
			{
                ...
                User.findOne(event.ownedby[0]).exec(function(err,user){
                    ...
                    event.iconbackground = (event.iconbackground && event.iconbackground!='') ? sails.config.master_url+'/event/backgroundurl/'+event.id : '';
